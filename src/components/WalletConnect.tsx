...
export const WalletConnect: React.FC = () => {
    ...
    const initializeWallet = async (walletWif: string) => {
        setLoading(true);
        try {
            await walletService.importWallet(walletWif);
            await updateWalletInfo();
            localStorage.setItem('wallet_wif', walletWif); // Save to storage
        } catch (e) {
            console.error(e);
    ...
    const handleCreate = async () => {
        setLoading(true);
        try {
            const newWif = await walletService.createWallet();
            setWif(newWif);
            await initializeWallet(newWif);
        } catch (e) {
            console.error(e);
    ...
    const handleDisconnect = () => {
        ...
        setWif('');
        setBalance(null);
        localStorage.removeItem('wallet_wif'); // Clear from storage
        window.location.reload(); // Reload to clear service state
    };
            ...
            {!address ? (
                <div className="space-y-4">
                    <button
                        ...
                        disabled={loading}
                        className="w-full bg-green-500 hover:bg-green-600 py-2 rounded font-semibold transition"
                    >
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            disabled={loading}
                            className="bg-blue-500 hover:bg-blue-600 px-4 rounded font-semibold transition"
                        >
                            Import
                        </button>
                    </div>
